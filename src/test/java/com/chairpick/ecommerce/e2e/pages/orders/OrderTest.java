package com.chairpick.ecommerce.e2e.pages.orders;
...
import com.chairpick.ecommerce.e2e.utils.*;
...
public class OrderTest {
    ...
    public void shouldMakeAnOrderAndDeliver() throws InterruptedException {
        ...
        ProfilePage.Card card2 = profilePage.getCardFromSection(ProfilePage.OrdersStatus.APPROVED, 1, 1);
        Assertions.assertEquals("Chair 1", card2.getProductName());
        Assertions.assertEquals("R$ 10,00", card2.getProductPrice());
        Assertions.assertEquals("1", card2.getProductAmount());
        Assertions.assertEquals("Entregue", card2.getProductStatus());
        Assertions.assertEquals("R$ 10,00", card2.getProductSubtotal());
        Assertions.assertEquals("R$ 8,90", card2.getProductFreight());
        Assertions.assertEquals("R$ 18,90", card2.getProductTotal());
    }
    ...
    public void shouldReproveOrderExcluding() throws InterruptedException {
        ...
        OrderItemsPage orderItemsPage = (OrderItemsPage) orderActions.clickAction("view_items");
        wait.until(ExpectedConditions.urlToBe(BASE_URL + "admin/orders/1"));
        Assertions.assertEquals(OrderStatus.REPROVED.getDescription(), orderItemsPage.getStatusOfRow(1));
        ...
        indexPage.accessHeaderOption(IndexPage.HeaderOptions.PROFILE);
        wait.until(ExpectedConditions.urlToBe(BASE_URL + "customers/1"));
        wait.until(ExpectedConditions.presenceOfAllElementsLocatedBy(By.cssSelector("section.REPROVED")));
        profilePage.scrollIntoSectionOfOrders(ProfilePage.OrdersStatus.REPROVED);

        ProfilePage.Card card2 = profilePage.getCardFromSection(ProfilePage.OrdersStatus.REPROVED, 1, 1);
        Assertions.assertEquals("Chair 1", card2.getProductName());
        Assertions.assertEquals("R$ 10,00", card2.getProductPrice());
        Assertions.assertEquals("1", card2.getProductAmount());
        Assertions.assertEquals("Reprovado", card2.getProductStatus());
        Assertions.assertEquals("R$ 10,00", card2.getProductSubtotal());
        Assertions.assertEquals("R$ 8,90", card2.getProductFreight());
        Assertions.assertEquals("R$ 18,90", card2.getProductTotal());

        modal = profilePage.clickToCancelOrder(ProfilePage.OrdersStatus.REPROVED, 1);
        modal.confirm();
        wait.until(ExpectedConditions.urlToBe(BASE_URL + "customers/1"));

        wait.until(ExpectedConditions.not(ExpectedConditions.presenceOfAllElementsLocatedBy(By.cssSelector("section.REPROVED"))));
    }

    @Test
    public void shouldMakeAnOrderPayingOnlyWithCoupons() throws InterruptedException {
        CouponInitializer couponInitializer = new CouponInitializer(seeder);
        Long customerId = usersInitializer.getCustomerId();
        Long firstPromoCouponId = couponInitializer.insertCouponForCustomerWithValue(customerId, 18.90, CouponType.PROMOTIONAL);
        Long secondPromoCouponId = couponInitializer.insertCouponForCustomerWithValue(customerId, 10.0, CouponType.PROMOTIONAL);
        Long swapCouponId = couponInitializer.insertCouponForCustomerWithValue(customerId, 8.90, CouponType.SWAP);
        driver.get(BASE_URL);
        wait.until(ExpectedConditions.urlToBe(BASE_URL));
        ...
        wait.until(ExpectedConditions.urlToBe(BASE_URL + "customers/1/orders/payment"));
        Thread.sleep(3000);
        Assertions.assertEquals(2, orderPaymentPage.getAllPromotionalCoupons().size());
        Assertions.assertEquals(1, orderPaymentPage.getAllSwapCoupons().size());

        orderPaymentPage.selectPromotionalCouponWithId(firstPromoCouponId);
        Double creditCardsTotalValue = orderPaymentPage.getValueInActiveCreditCards();
        Thread.sleep(3000);
        Assertions.assertEquals(0.0, creditCardsTotalValue);
        orderPaymentPage.selectPromotionalCouponWithId(firstPromoCouponId);
        creditCardsTotalValue = orderPaymentPage.getValueInActiveCreditCards();
        Assertions.assertEquals(18.90, creditCardsTotalValue);
        orderPaymentPage.selectPromotionalCouponWithId(secondPromoCouponId);
        creditCardsTotalValue = orderPaymentPage.getValueInActiveCreditCards();
        Assertions.assertEquals(8.90, creditCardsTotalValue);
        orderPaymentPage.selectSwapCouponWithId(swapCouponId);
        creditCardsTotalValue = orderPaymentPage.getValueInActiveCreditCards();
        Assertions.assertEquals(0.0, creditCardsTotalValue);
        String messageGenerateSwapCoupon = orderPaymentPage.getExcessCouponValueMessage();
        Assertions.assertEquals("", messageGenerateSwapCoupon);
        orderPaymentPage.selectPromotionalCouponWithId(secondPromoCouponId);
        orderPaymentPage.selectPromotionalCouponWithId(firstPromoCouponId);
        creditCardsTotalValue = orderPaymentPage.getValueInActiveCreditCards();
        Assertions.assertEquals(0, creditCardsTotalValue);
        messageGenerateSwapCoupon = orderPaymentPage.getExcessCouponValueMessage();
        Thread.sleep(2000);
        Assertions.assertEquals("Será gerado um cupom de troca no valor de R$ 8,90", messageGenerateSwapCoupon);
        ProfilePage profilePage = orderPaymentPage.confirmOrder();
        wait.until(ExpectedConditions.urlToBe(BASE_URL + "customers/1"));
        Thread.sleep(2000);
        profilePage.scrollIntoSectionOfOrders(ProfilePage.OrdersStatus.PENDING);
        ...
        profilePage.accessHeaderOption(IndexPage.HeaderOptions.HOME);
        chairPage = indexPage.selectAnyProduct();
        cartConfirmationPage = chairPage.clickBuyButton(wait);
        wait.until(ExpectedConditions.urlToBe(BASE_URL + "customers/1/cart/confirm"));
        cartConfirmationPage.clickConfirmButton();
        wait.until(ExpectedConditions.urlToBe(BASE_URL + "customers/1/orders/payment"));
        Assertions.assertEquals(1,orderPaymentPage.getAllSwapCoupons().size());
        double swapCouponsValues = orderPaymentPage.getSwapCouponsValues();
        Assertions.assertEquals(8.90, swapCouponsValues);
    }
